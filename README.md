# Ex2.ReteNeurale
Rete Neurale per una regressione lineare
Il codice  Python creerà e addestrerà una semplice rete neurale per gestire un problema di regressione lineare: dato un certo dataset di punti su un piano cartesiano, la rete neurale addestrata sarà in grado di riconoscere l’andamento. Useremo TensorFlow/Keras per definire e addestrare il modello.
...
Poi si addestra il modello utilizzando il metodo fit(). Forniamo i dati di input (X), i dati di output target (y), il numero di epoche (quante volte il modello vedrà l'intero dataset) e la dimensione del batch.
